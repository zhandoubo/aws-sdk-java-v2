...
public class AssumeRoleIntegrationTest extends IntegrationTestBaseWithIAM {
    ...
    public static void setup() {
        ...
        // Delete any old credentials for the user. We don't want to hit our limit.
        iam.listAccessKeysPaginator(r -> r.userName(USER_NAME))
           .accessKeyMetadata().stream()
           .filter(key -> key.createDate().isBefore(Instant.now().minus(1, HOURS)))
           .forEach(key -> iam.deleteAccessKey(r -> r.accessKeyId(key.accessKeyId())));
